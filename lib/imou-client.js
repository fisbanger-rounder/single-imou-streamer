...
export class ImouClient {
  ...
  /**
   * List all cameras/channels bound to the Imou account.
   * Uses deviceBaseList with bindId cursor pagination.
   */
  async listDevices(type = "bindAndShare", limit = 128) {
    const token = await this.getAccessToken();
    const devices = [];
    let bindId = -1;
    while (true) {
      const data = await this.#call("deviceBaseList", {
        token,
        bindId,
        limit,
        type,
      ...
      bindId = list[list.length - 1].bindId;
    }
